import os
# Force OpenCV to use TCP for RTSP to prevent UDP packet loss and NAL unit errors
...
cp_latest_jpeg  = None          # latest JPEG bytes from CP Plus
cp_latest_frame = None          # latest raw cv2 frame from CP Plus
cp_stream_active = False

def cp_capture_loop():
    """Background thread: reads CP Plus RTSP and keeps cp_latest_jpeg fresh."""
    global cp_cap, cp_latest_jpeg, cp_latest_frame, cp_stream_active
    consecutive_fail = 0
    log.info("[CP] Capture loop started")
    while True:
        with cp_frame_lock:
            cap = cp_cap
        ...
        if segs_to_draw:
            ...
            for seg in segs_to_draw:
                ...
                cv2.rectangle(draw_frame, (lx, ly - th - 12), (lx + tw + 16, ly + baseline + 12),
                              (255, 255, 255), -1)
                # Dark blue text (BGR)
                text_color = (139, 0, 0)
                cv2.putText(draw_frame, label, (lx + 8, ly),
                            cv2.FONT_HERSHEY_SIMPLEX, font_scale, text_color, thickness, cv2.LINE_AA)

            # Blend overlay (transparent fill) onto draw_frame
            cv2.addWeighted(overlay, 0.38, draw_frame, 0.62, 0, draw_frame)

        _, buf = cv2.imencode(".jpg", draw_frame, [cv2.IMWRITE_JPEG_QUALITY, 70])
        with cp_frame_lock:
            cp_latest_jpeg = buf.tobytes()
            cp_latest_frame = frame.copy()
        time.sleep(0.005)   # ~30 fps
...
def gen_cp_frames():
    """MJPEG generator for CP Plus stream."""
...
try:
    ...
    ocr_worker = None
    log.warning("ocr_processor module not found, OCR disabled")
...
def ind_capture_loop():
    global ind_latest_jpeg, ind_video_cap, ind_video_active
...
def gen_ind_frames():
    ...
    while True:
        ...
        yield (b"--frame\r\nContent-Type: image/jpeg\r\n\r\n"
               + (jpeg if jpeg else placeholder) + b"\r\n")
        time.sleep(0.005)
...
ZONE_CONFIG = {
    "inner1": {"name": "Inner Zone 1", "inspect_time": 15.0},
    "inner2": {"name": "Inner Zone 2", "inspect_time": 8.0},
    "outer1": {"name": "Outer Zone 1", "inspect_time": 15.0},
    "outer2": {"name": "Outer Zone 2", "inspect_time": 8.0},
}
...
try:
    ...
    log.info("Loading Zone YOLO model (shi-seq-v2-100-epochs.pt)...")
    zone_model = YOLO(r"Models\shi-seq-v2-100-epochs.pt")
except Exception as e:
...
def _save_single_zone_image(zone_name, img_bytes):
    """Instantly save a zone image to disk the moment it is captured."""
    today_str = datetime.now().strftime("%Y-%m-%d")
    serial_str = current_cycle.get("serial", "temp")
    if serial_str == "------" or not serial_str.strip():
        serial_str = "temp"

    day_dir = os.path.join(BASE_DIR, "lid_data", today_str)
    final_dir = os.path.join(day_dir, serial_str)
    os.makedirs(final_dir, exist_ok=True)

    img_path = os.path.join(final_dir, f"{serial_str}_{zone_name}.jpg")
    with open(img_path, "wb") as f:
        f.write(img_bytes)
    log.info(f"[IMAGE] Saved {zone_name} → {img_path}")
...
def zone_inference_loop():
    ...
    while True:
        ...
        try:
            # ── Run YOLO inference ────────────────────────────────────────────
            results = zone_model(frame, verbose=False)

            # Clean frame for disk/PDF (no annotations)
            ...
            with lock:
                ...
                for zone in ALL_ZONES:
                    ...
                    # ── IDLE: waiting for operator to present this zone ───────
                    if status == "idle":
                        if is_detected:
                            if zone == expected_zone:
                                ...
                                if expected_zone:
                                    current_cycle["instruction"] = f"PLEASE INSPECT {ZONE_CONFIG[expected_zone]['name'].upper()} FIRST"
                                    current_cycle["instruction_color"] = "orange"
                    ...
                    # ── DETECTING: hand on part, progress filling over time ───
                        ...
                        if is_detected:
                            ...
                            if progress >= 100:
                                # Full inspection time reached → ask to remove hand
                                current_cycle[f"zone_{zone}_status"] = "ready_to_capture"
                                zt[zone]["capture_start"] = now
                                current_cycle["instruction"] = (
                                    "CAPTURING PICTURE KEEP PART CORRECT AND TAKE HAND OUT"
                                )
                                current_cycle["instruction_color"] = "orange"
                            ...
                            if gap > 1.5:
                                ...
                                if total_on_part >= MIN_INSPECT_TIME:
                                    # ── EARLY FINISH OK ──────────────────────
                                    # Operator finished faster than inspect_time
                                    # but was on part long enough → go to capture
                                    current_cycle[f"zone_{zone}_status"] = "ready_to_capture"
                                    zt[zone]["capture_start"] = now
                                    current_cycle["instruction"] = (
                                        "CAPTURING PICTURE KEEP PART CORRECT AND TAKE HAND OUT"
                                    ...
                                    current_cycle["instruction_color"] = "orange"
                                    log.info(f"[ZONE] {zone} early finish ({total_on_part:.1f}s), capturing")
                                else:
                                    # ── TOO EARLY — hand slipped ─────────────
                                    # Below minimum → error, operator must redo
                                    zt[zone]["accumulated"] += (
                                        zt[zone]["last_seen"] - zt[zone]["detect_start"]
                                    )
                    ...
                    # ── READY_TO_CAPTURE: waiting for clean frame after hand removal ─
                    elif status == "ready_to_capture":
                        elapsed_wait = now - zt[zone]["capture_start"]
                        if elapsed_wait >= CAPTURE_DELAY:
                            ...
                            _save_single_zone_image(zone, clean_jpeg)
                            ...
                            if nxt:
                                current_cycle["instruction"] = (
                                    f"CAPTURED! GO TO {ZONE_CONFIG[nxt]['name'].upper()}"
                                )
                                current_cycle["instruction_color"] = "blue"
                            else:
                                # All zones done
                    ...
                    # ── ERROR: recoverable — resume if zone detected again ────
                        ...
                        if is_detected:
                            if zone == expected_zone:
                                ...
                                zt[zone]["detect_start"] = now
                                zt[zone]["last_seen"] = now
                                # Keep accumulated time so progress doesn't reset
                                current_cycle["instruction"] = (
                                    f"INSPECTING {cfg['name'].upper()} — KEEP HAND ON PART"
                                )
                                current_cycle["instruction_color"] = "blue"
...
def _maybe_finalize_cycle():
    ...
    # --- File Saving & PDF Generation Logic (SSP Style) ---
    today_str = datetime.now().strftime("%Y-%m-%d")
    ...
    base_path = os.path.join(day_dir, serial_str)
    if os.path.exists(base_path):
        folder_name = f"{serial_str}_{timestamp_str}"
    ...
    # Save the images from current_cycle["zone_images"]
    saved_images = {}
    for zone, img_data in current_cycle.get("zone_images", {}).items():
        if img_data is not None:
            img_path = os.path.join(final_dir, f"{folder_name}_{zone}.jpg")
            # If img_data is raw bytes (jpeg), save it directly
    ...
    # Inject OCR raw crop directly from memory
    if "ocr_raw_crop" in current_cycle:
        crop_dir = os.path.join(final_dir, "crop")
        os.makedirs(crop_dir, exist_ok=True)
        crop_path = os.path.join(crop_dir, f"raw_crop_{timestamp_str}.jpg")
